...
class ProfileActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //updateDriverProfile when the activity is created
        updateDriverProfile()
    }
    ...
    private fun updateDriverProfile() {
        ...
        firestore.collection("users").document(userId)
            ...
            .addOnSuccessListener { documents ->
                ...
                if (trips.size >= 3) {
                    //ML classifier's classify method instead of rule-based classifyDriver
                    ...
                    if (lastTrip != null) {
                        // use the personalised feedback from DriverClassifier
                        val ruleBasedClassifier = DriverClassifierRule()
                        binding.driverFeedbackText.text =
                            ruleBasedClassifier.getPersonalisedFeedback(category, lastTrip)
                    }
                } else {
                    //Not enough trips to generat
                    binding.avgEfficiencyScoreText.text = "--"
